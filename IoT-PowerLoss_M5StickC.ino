...
void powerLoss_detected() {    
    ...
    if(sleep_count >= 5) //sleep device after 5 * [checkDelay] msec
    {
        sleep_count = 0;
...
void loop() {
    ...
    if(startTime < (loopTime - checkDelay)) //Cehck power status every [checkDelay] msec
    {
        if(M5.Axp.GetVBusVoltage()<4)
        {
            powerLoss_detected();
